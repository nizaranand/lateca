...
class Permalink_Editor
{
	...
	function get_post_by_custom_permalink( $permalink, $exclude = false, $suffix = '' )
	{
		...
		if ( $post_types = get_post_types( array( 'public' => true ), 'names' ) ) {
			if ( $posts = get_posts( array(
				...
				// Lookup the permalink with a slash appended, just in case...
	...
	function get_sample_permalink_html( $html, $id, $new_title, $new_slug )
	{
		...
		$permalink = apply_filters( 'get_custom_permalink_sample', $permalink, $post );
	...
	{
		if ( ! isset( $_POST[$this->tag . '_nonce'] ) || ! wp_verify_nonce( $_POST[$this->tag . '_nonce'], plugin_basename( __FILE__ ) ) ) {
			return $post_id;
		}
		...
		foreach ( $fields AS $field ) {
			if ( isset( $_POST[$field] ) ) {
				$value = $this->unique_custom_permalink(
					$post_id,
					$this->reformat_permalink( $_POST[$field] )
				);
	...
	/**
	 * Ensure the custom permalink is valid, and if not output an error message
